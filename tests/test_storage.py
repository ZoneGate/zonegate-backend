...
@pytest.mark.asyncio
async def test_a_policy_config_saved_with_the_retired_window_keeps_its_categories(memory_store):
    """Removing the window must not silently reset a deployment's category map."""
    memory_store._run_sync(
        lambda: memory_store._db.kv_put(
            memory_store.NS_INDEXES,
            memory_store.KEY_POLICY_CONFIG,
            b'{"restricted_categories":{"WEAPONS":"ROLE_SECURITY_OFFICER"},"window_start_hour":6,"window_end_hour":20}',
        )
    )
    ...
    assert reread is not None
    assert reread.restricted_categories == {"WEAPONS": "ROLE_SECURITY_OFFICER"}
